...
console.log('\n  omega x actuator sweep, lag pi/4 and pi/2, 12-seg chain\n');
console.log('  omega   lag    actuator            net m/s   CoM m/s     eff      CoT');
for (const omega of [1, 2, 4, 8]) {
  for (const [ln, lag] of [['pi/4', Math.PI/4], ['pi/2', Math.PI/2]]) {
    for (const [label, opts] of [
      ...
      const r = trial(serpent({ lag, omega }), opts);
      if (r.err) { console.log(`  ${String(omega).padStart(5)}   ${ln}   ${label.padEnd(16)}  ERROR ${r.err}`); continue; }
      const cot = r.work / Math.max(r.net * 15, 1e-6);
      console.log(`  ${String(omega).padStart(5)}   ${ln}   ${label.padEnd(16)}  ${r.net.toFixed(4).padStart(7)}   ${r.com.toFixed(3).padStart(7)}   ${r.eff.toFixed(3).padStart(6)}  ${(r.work>0?cot.toFixed(0):'n/a').padStart(7)}`);
    }
  }
}
console.log('');
