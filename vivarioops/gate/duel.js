...
export async function runDuelGate() {
  ...
  g.assertion('L2-10', 'The three frozen residents load, hash as recorded, and differ', (t) => {
    ...
    // 9: A1/A2/A3, then GENOME_V 5 (organ placement), then the MUSCLE_STRESS
    // ceiling/gain split (200 -> 2e6 on the ceiling only). Each moves every genome
    // hash, so each is a re-freeze.
    ...
    t.eq(W1_SLICE.faunaVersion, 12, 'faunaVersion bumps on any change that moves genome hashes; the latest is GENOME_V 9');
  ...
  return {
    ...
    obligations: [
      // ── REVIEWED 2026-08-14 against design/PLAN.md. THE MATRIX IS NO LONGER
      // ALL ZEROS, and the reason is that the TASK was wrong rather than the
      // creatures. Read the first two entries together: this suite has now
      // blamed the drag law, then aiming, and been wrong both times.
      'THE DUEL TASK WAS THE DEFECT, AND IT IS FIXED. `duelDuration` 15 -> 90 s, derived by tools/_zduelfit.mjs from the only quantity that decides whether a duel can happen at all: reachSum / (cruiseA + cruiseB), the time a pair needs to close its own touching range. The old task asked for a 20-30 cm crossing in 15 s from animals whose best covers 7.2 cm — roughly forty times harder than the 90 s / 8 cm beacon trial the same animals win. At 90 s, 96% of champion pairs can meet (was 14%) and 26% of random pairs (was 0%). Separation also gained a ceiling at what the pair can actually cross, and `duelSetup` now returns `unreachable` — so "the task was impossible" and "the animal failed to catch" are no longer conflated, which is what let an impossible task read as a failure of the creatures.',
      'RESULT, SAME HARNESS, RANDOM POOL AS CONTROL: campaign champions 4 captures in 84 duels, random corpus 0 in 84. Median time to capture 7.3 s, fastest 6.1. C2\'s matchup matrix was all zeros for three sittings and is not any more. The random corpus still mostly fails, and that is CORRECT — an unselected creature should not catch anything.',
      'SUPERSEDED, TWICE, AND BOTH DIAGNOSES ARE WORTH REMEMBERING AS A PATTERN. (1) "The blocker is the DRAG LAW" — the drag law was fixed and the duels still did not capture. (2) "They do not pursue: closing is FLAT at 0.16-0.21 cm across a 4x separation range, so it is drift, not approach. C2 needs pursuit, pursuit needs orientation, and orientation is the open problem. LOCOMOTION IS NO LONGER THE BLOCKER; AIMING IS." The closing measurement was real and reproduces; the INFERENCE was wrong, because at 15 s neither aiming nor arriving was possible and the second failure hid behind the first. The lesson that survives: derive the task from measured capability, never from a spec constant.',
      ...
      'DUEL INSTABILITY IS RESOLVED AND THE CONTAINMENT ESCAPE IS UNPROVEN EITHER WAY. ~16-33% of duels used to abort as unstable and it is now ZERO across 84; the cause was the peak-speed tail meeting a wall, not the arena refactor and not creature-creature contact (a lone creature centred in a private tank was 0/10, two in a shared arena 7/45). One creature was also measured LEAVING the tank through the surface collider. Neither has been re-observed since the density pin and the actuator bound — but nothing re-ran the repro either, so this is "not seen lately", not "cannot happen". The same gap exists for the added-mass tear-apart: _myria2.json no longer reproduces it, so a repro must be rebuilt before anyone leans on either claim.',
      'C2: 11 §12.5 non-transitivity is REPORTED, not asserted. With three residents there is exactly one candidate cycle, so its absence is weak evidence; it earns an assertion once the fauna is large enough for the claim to have teeth — design/PLAN.md Phase 6.',
      'THE CAPABILITY CARD AND DUEL REPLAY (21 §6) ARE STILL NOT BUILT, AND THE REASON HAS CHANGED. It was deferred because a matchup grid of three all-zero rows would have shown the player nothing and 21 §6.2 asks it to "flag surprises" — there were none to flag. There are now. What it waits on instead is design/PLAN.md Phase 6, and it carries an obligation the card itself cannot discharge: a pairwise win rate measured in ONE arena, at ONE separation, with ONE starting orientation and no third party present does not become a context-free truth by having come from physics (VIVARIUM_00 P3). Transfer error to L3 must be measured, not assumed away.',
    ],
  };
}
