...
## 0.8.0 — 2026-07-29 — C1 sensors and probes · C2 duels (GATE GREEN)

**75 assertions, 71 passed, 0 failed, 4 pending. 2019 checks.** Up from 57/1538.

The bridge exists. A creature can be compiled into a measured record, three
frozen residents ship with the world, and two creatures can fight in one physics
world. **And the fight measures nothing** — see the blocker below, which is the
most important thing in this entry.

### C2 — the blocker, stated first because it decides the next session

30 §5 C2's checkpoint is *"you compile a creature, watch three fights, and can
say what it is good and bad at"*. **It is not answerable.** Every resident duel
is a stalemate: 0 captures over 9 duels, **median closing distance 0.00 m** over
15 s against a start separation of several metres.

It was chased rather than tuned around:

- **Capture is reachable.** `tools/c2sweep.js` measures 6/45 captures when the
  pair starts at half the reach sum. The root-contact rule and the harness are
  correct; the locomotion is not.
- **The root cause is the standing B3/B5 buoyancy obligation coming due.** In the
  duel tank, **6 of 10 creatures reach the floor and 2 reach or pass through the
  surface within 15 s**. They spend the fight pinned against a boundary, sliding,
  not swimming. Solo probes avoid this by measuring at gravity zero (C1); a duel
  cannot, because the tank is where the contact happens. 02 §7's density range
  0.15–1.8 against `mediumDensity` 1.0 has to be decided before a matchup matrix
  is worth storing.
- **One creature left the tank** — y = 16.5 against a surface at y = 12.
  Containment is not guaranteed at the speeds the B3 tail reaches.

### Added — C2
- `engine/l2/duel.js` — the duel probe. Canonical pair seeding (03 §2), root-body
  capture, 15 s cap, both directions recorded, reduction to a symmetric
  `PairMatchup`. All three of 10 §7's exploit guards are in force.
- `engine/l2/fauna.js` — the fauna loader. Owns exactly the two fields
  `compileSolo` leaves null, `id` and `vs`, and refuses a fauna with a hole
  rather than defaulting it: a zero would read as a measurement.
- `worlds/w1_residents.js` — **three frozen residents**, generated by
  `tools/c2residents.js` and chosen for *measured* spread over 40 viable
  candidates rather than at random. *Oligodermops crassus* (2 bodies, 2.0 kg,
  cruise 2.05 m/s), *Mesopterus torquens* (37 kg, turns 10x better than anything
  else in the corpus), *Mesodermops elongatus* (38 kg, effectively cannot turn).
- `gate/duel.js` — L2-10..L2-18.

### Changed — the arena, and why B4b tiled
`createSimulation` built its own `RAPIER.World` per call, which is why the tank
tiles six private tanks and why creatures there cannot touch (open decision 5).
A duel is contact between two creatures and therefore needs one world, so
**`createArena` is extracted** and `createSimulation` attaches to one. With no
arena passed it builds a private one in the old construction order, so every B3
number is unchanged. `step()` splits into `applyForces()` / `advanceClock()` and
`arena.stepAll()` applies forces from *all* occupants before the single solve —
apply-then-solve per creature would make a duel depend on argument order, which
is exactly what K4 forbids.

**The refactor is exonerated by measurement, not by assertion.** `tools/c2stab.js`
isolates the ~16% duel instability across four conditions: lone creature centred
in a private tank **0/10**, off-centre **1/10**, alone in a shared arena **1/10**,
two in a shared arena **7/45**. Neither the refactor nor creature-creature contact
is the cause — it is the B3 peak-speed tail meeting a wall.

### Closed — a carried obligation
`W1_SLICE.residents` placeholder hashes are replaced with **real genome hashes**
and `faunaVersion` is bumped 1 -> 2, so every record compiled before C2 is
correctly invalidated. K5 was restating the old placeholder ids as literals and
had silently stopped testing what it names; it now derives its permutations from
the fixture.

### Spec defects found at C2
- **11 §6's start separation does not fit its own tank.** It asks for
  `k x (reachA + reachB)`, k in {2..6}; over a viable corpus that is a median of
  **29 m inside a 16 x 24 x 16 m tank**, and 03 §4 puts L3 engagement at
  `4 x (reachA + reachB)` = 28 m in that same tank. Both are unsatisfiable.
  Clamping made every k identical, so three "deterministic variations" (11 §4)
  were the same fight; the k set is now **mapped onto the room the tank has**,
  order preserved, and `engagementRadius` records what was achieved.
- **11 §6 does not resolve simultaneous strikes.** It lists three termination
  conditions as "whichever comes first" and is silent on two arriving together.
  Only a symmetric answer survives K4, so mutual root contact is a stalemate.
- **11 §12.5 non-transitivity is reported, not asserted.** With three residents
  there is exactly one candidate cycle, so absence is weak evidence. It earns an
  assertion at step F.

### Deliberately not built
**The capability card and duel replay UI (21 §6).** A matchup grid of three
all-zero rows shows the player nothing, and 21 §6.2 asks it to "flag surprises
against expectation" — there are none to flag. Deferred until the blocker is
resolved.

### Gate discipline
L2-15 caught a defect in its own session's code: the first version of the
separation fix computed available room against the worst horizontal axis
regardless of bearing, throwing away the tank's 22.6 m diagonal, and every
bearing then produced the same room. That is the B4 lesson — *an assertion whose
corpus cannot violate it asserts nothing* — catching a fixture instead of an
assertion.

**Mutation test incomplete at delivery.** `tools/_mut_c2.mjs` seeds 26 defects;
9 were caught with 0 escapes before the session ended. **C2's green is therefore
not yet accepted** under the project's own standing rule. Finishing that run is
