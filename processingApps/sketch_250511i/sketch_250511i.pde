// "Luminous Streams Refined v4" - Clusters as soft gradient halos (darker, subtler)
...
class Cluster {
  ...
  void display() {
    ...
    blendMode(ADD);
    noStroke();
    // draw three concentric, decreasing-opacity halos with lower lightness
    for (int i = 0; i < 3; i++) {
      float f = 1 - i * 0.3;
      float r = size * f;
      // reduce brightness to 80 and alpha max to 15
      float a = (1 - p) * 15 * f;
      fill(baseHue, 60, 80, a);
