...
float speed = 2;              // Reduced speed of particle movement for thicker trails
...
float trailLength = 300;        // Maximum length of the particle trail
float backgroundAlpha = 100;     // Background fading alpha
...
int applyColorMask(float x, float y) {
  ...
  // Using an orange palette with subtle transitions
  float r = (sin(maskX * TWO_PI + maskOffsetX * 0.1) * 80 + 150) + (sin(maskY * TWO_PI + maskOffsetY * 0.1) * 50);
  float g = (cos(maskY * TWO_PI + maskOffsetY * 0.2) * 30 + 50) + (cos(maskX * TWO_PI + maskOffsetX * 0.15) * 30);
  float b = (sin(maskX * TWO_PI + maskOffsetY * 0.3) * 80 + 120) + (cos(maskY * TWO_PI + maskOffsetX * 0.25) * 10);

  // Keeping the values within range
  r = constrain(r, 0, 255);
  g = constrain(g, 50, 150); // Reduced green
  b = constrain(b, 50, 100); // Reduced blue

  return color(r, g, b, 150); // Soft orange tones with alpha for fading
}
