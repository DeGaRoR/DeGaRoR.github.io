...
int applyColorMask(float x, float y) {
  ...
  float r = sin(maskX * TWO_PI + maskOffsetX * 0.1) * 127 + 128;
  float g = cos(maskY * TWO_PI + maskOffsetY * 0.2) * 127 + 128;
  float b = sin(maskX * TWO_PI + maskOffsetY * 0.3) * 127 + 128;
...
class Particle {
  ...
  Particle(float x, float y) {
    ...
    velocity = new PVector(random(-speed, speed), random(-speed, speed));
...
void updateColorMask() {
  maskOffsetX += 0.005;  // Increase the speed of the color mask animation
  maskOffsetY += 0.005;  // Increase the speed of the color mask animation
}
