...
|--------|---------------|-----------|-----------|-------|
| Old S8 spec (draft) | 0.34 mol CH₂O/hr/person | 6 | not specified | not computed |
| Biosphere validation (past chat) | 0.947 mol/hr/person | 6 | 1% | 82 kW |
| S8 greenhouse template (draft) | not specified | — | 28% | ~3 kW implied |

The old-S8 metabolic rate (0.34 mol/hr) gives only 1013 kcal/day — a
starvation diet. The S8 template's 28% efficiency is the theoretical
...
**S8 makes this manageable.** The player:
...
Without S8, building 5× (8 units each) = 40 units on a flat
canvas would be unmanageable. With S8, it's 5 group boxes plus
...
1. **M10 power requirement (~85 kW) — RESOLVED:**
   ...
   (b) S8 group templates make building at scale manageable —
...
#### 4.3.5 Power Budget Table (§S10c-10, add after population timeline)
...
built. S8 group templates make this manageable.
```
...
| S8 | ~15 lines | Greenhouse template efficiency, editableParams field, human template rates |
| S10 | ~40 lines | Metabolic rates, M10 mission rewrite, power budget table, open question resolution |
| S0 (NNG) | 0 | No NNG changes (groups already covered in S8 amendment) |

### New S8 schema addition

The `editableParams` field on `TemplateUnit` is a small but important
addition to the S8 spec. It enables "locked group with selective
...
- S10 T-3: R_METABOLISM registered (rates are comments, not tested directly)
- S8 T-GR-INVARIANT: physics unchanged by grouping

The 85 kW number should be validated during S10c session 3 integration
