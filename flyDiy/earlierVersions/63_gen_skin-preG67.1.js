...
function genSkin(def) {
  ...
  // ---- 3. wing --------------------------------------------------------
  // Lifted out whole by G67.1 (see genWingInto): the wing is the half of this
  // file the cage still needs, so it is a function both callers reach rather
  // than a stretch only this one can. What comes back is what the hinge table
  // below still reads — the band ends and the hinge fractions.
  const emitLoft = (rows, mesh, vOf, flip, close) =>
    genEmitLoft(rows, mesh, vOf, flip, close, B);
  const capLoft = genCapLoft;
  const { aStart, fEnd, FLAP_ON, FLAP_HINGE, AIL_HINGE, sparF } =
    genWingInto(def, { B, skin, pitot, canopy, ctrl: CTRL_MESH });
  // ---- 4. empennage ---------------------------------------------------
