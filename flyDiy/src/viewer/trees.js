...
(() => {
  ...
  const FADE_VS = [
    ...
    '  vec3 _fp = (modelMatrix * vec4(batchingMatrix[3].xyz, 1.0)).xyz;',   // the cover ring's batches (G574): an instance's origin
    '#else',
  ...
  // A BATCH HAS NO PER-INSTANCE ATTRIBUTE (G574): the cover ring draws its rocks and debris as BatchedMeshes, one
  ...
  function hookLeaf(mat, isLeaf, tint, cut) {
    ...
    mat.onBeforeCompile = sh => {
      ...
      { let vs = sh.vertexShader.replace('#include <begin_vertex>', '#include <begin_vertex>\nvAoV = aoV;');
        ...
        // ...which a batch (the cover ring's shrubs, G574) does not have: a multi-draw draws each instance as its own
