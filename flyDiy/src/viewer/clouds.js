// ============================================================
// THE CLOUDS — a volumetric layer over the world (CLOUDS C1, 2026-09-15).
//
...
// cloud's own mean distance, then tone-mapped in the pass because the
// resolve target is display-space (aa_resolve.js).
//
// THE MARCH runs at half resolution (GRAPHICS `clouds`: off / half / full)
// into its own target, stopped at the scene's depth (the resolve target's
// depth texture - the mountains poke through the layer, the aeroplane in
// front of a cloud is in front of it), then composites over the frame
// before the resolve (aa.setOverlay). The noise is baked on the GPU into
// two 3D textures at first use, a few slices a frame.
//
...
var CLOUDS = (function () {
  ...
  const S = { mode: 'half', steps: 48, lightSteps: 5, sigma: 0.08, seed: 7, base: 0, thick: 0, driftK: 1, powder: 0.6, ambK: 1, sunK: 1,
              detail: 0.5, g: 0.75, period: 6000, detailPeriod: 900, ms: 0.5, bakeSlices: 6, maxKm: 60, curl: 0.3 };
  const NB = 128, ND = 32;                 // the base and detail noise sides
  ...
  let lay = null, dayRef = null, lastCover = 0, stats = { ms: 0, slicesBaked: 0, cover: 0 };
  const drift = [0, 0];
  const U = {
    ...
    uPeriods: { value: null }, uSun: { value: null }, uSunCol: { value: null }, uMoon: { value: null }, uMoonCol: { value: null },
    uAmbTop: { value: null }, uAmbBot: { value: null }, uScale: { value: 1 }, uSteps: { value: null }, uFrame: { value: 0 },
    uDials: { value: null }, uDials2: { value: null }, uCloudTex: { value: null }, uTexel: { value: null },
  };

  // ---- the noise bake (GLSL3, into a 3D target a slice at a time) ----------
  const NOISE_GLSL = `
  ...
  const marchFrag = () => `precision highp float; precision highp sampler3D;
    ${GLSL3_OUT}
    varying vec2 vUv;
    uniform sampler3D uNoise, uDetail; uniform sampler2D uWeather, uDepth;
    ...
    uniform vec4 uSun, uMoon;   // direction xyz, w unused
    uniform vec3 uSunCol, uMoonCol, uAmbTop, uAmbBot;   // sun units
    uniform float uScale, uFrame; uniform vec2 uSteps; uniform vec4 uDials;   // dials: powder, g, ms decay, maxKm
    ${ATMO_GLSL()}
    float remap(float v, float a, float b, float c, float d) { return c + (clamp((v - a) / (b - a), 0.0, 1.0)) * (d - c); }
    ...
    // density at p (0..1 x coverage); cheap = no detail (the light march)
    ...
    void main() {
      ...
      if (abs(d.y) < 1e-5) { if (o.y < yb || o.y > yt) discard; t0 = 0.0; t1 = uDials.w * 1000.0; }
      else {
      ...
      // grazing path (tens of km through the layer) is marched at a length that resolves a cloud and
      // grows with the distance (a far cloud is small on screen); empty air is crossed in strides -
      ...
      float N = uSteps.x;
      float dt0 = clamp((t1 - t0) / N, 24.0, 160.0);
      // a jitter a frame (interleaved gradient noise) breaks the banding
      ...
      for (float i = 0.0; i < 160.0; i += 1.0) {
        if (i >= N * 2.5 || T < 0.01 || t > t1) break;
        float dt = dt0 * (1.0 + t * 0.00006);
        vec3 p = o + d * t;
        vec4 w = weather(p);
  ...
  // the composite: the march's radiance tone-mapped like the dome, alpha over the frame in display space
  const COMP_FRAG = `varying vec2 vUv; uniform sampler2D uCloudTex; uniform vec2 uTexel;
    void main() {
  ...
  function init(r) {
    ...
    compMat = new THREE.ShaderMaterial({ uniforms: { uCloudTex: U.uCloudTex, uTexel: U.uTexel }, vertexShader: QUAD_VERT, fragmentShader: COMP_FRAG, depthTest: false, depthWrite: false, transparent: true, toneMapped: true, blending: THREE.NormalBlending });
    ready = true;
    return true;
  }
  ...
  function update(day, camera, world) {
    ...
    if (A) {
      ...
      // ambient: the sky's radiance (E/pi) at the top; below, the ground's reflected light + a share of the top's
      ...
      U.uAmbBot.value.set((0.3 * _E[0] + gnd(0)) * k, (0.3 * _E[1] + gnd(1)) * k, (0.3 * _E[2] + gnd(2)) * k);
    ...
    U.uSteps.value.set(S.steps, S.lightSteps); U.uDials.value.set(S.powder, S.g, S.ms, S.maxKm);
  ...
  function draw(r, camera, target) {
    ...
    if (!rt || rtW !== w || rtH !== h) {
      if (rt) rt.dispose();
      rt = new THREE.WebGLRenderTarget(w, h, { type: THREE.HalfFloatType, format: THREE.RGBAFormat, minFilter: THREE.LinearFilter, magFilter: THREE.LinearFilter, depthBuffer: false, stencilBuffer: false, generateMipmaps: false });
      rtW = w; rtH = h; U.uCloudTex.value = rt.texture; U.uTexel.value.set(1 / w, 1 / h);
    }
    frame++;
    ...
    const prevAC = r.autoClear; r.getClearColor(_cc); const prevCA = r.getClearAlpha();
    r.setRenderTarget(rt); r.setClearColor(0x000000, 0); r.clear(true, false, false);
    quad.material = marchMat; r.render(fsScene, fsCam);
    r.setRenderTarget(target); r.autoClear = false;
    quad.material = compMat; r.render(fsScene, fsCam);
    r.autoClear = prevAC; r.setClearColor(_cc, prevCA);
    if (t0) stats.ms = stats.ms * 0.9 + 0.1 * ((performance.now()) - t0);
  }
  // probe(): what the pass sees - the march target's mean alpha, a noise slice's mean, the depth at the centre
  ...
  function probe() {
    ...
    if (rt) { const px = new Uint16Array(4 * 16 * 16); renderer.readRenderTargetPixels(rt, (rtW >> 1) - 8, (rtH >> 1) - 8, 16, 16, px);
      let a = 0, c = 0; for (let i = 0; i < 256; i++) { a += px[i * 4 + 3]; c += px[i * 4]; } out.marchA = a / 256; out.marchR = c / 256; out.marchRaw = Array.from(px.slice(0, 8)); }
    if (!probeMat) {
      probeMat = new THREE.ShaderMaterial({ glslVersion: THREE.GLSL3, uniforms: { uNoise: U.uNoise, uDetail: U.uDetail, uWeather: U.uWeather, uDepth: U.uDepth, uWhat: { value: 0 } },
        vertexShader: QUAD_VERT, fragmentShader: `precision highp float; precision highp sampler3D; ${GLSL3_OUT}
          varying vec2 vUv; uniform sampler3D uNoise, uDetail; uniform sampler2D uWeather, uDepth; uniform float uWhat;
          void main() { if (uWhat < 0.5) gl_FragColor = texture(uNoise, vec3(vUv, 0.5)); else if (uWhat < 1.5) gl_FragColor = texture(uDetail, vec3(vUv, 0.5)); else if (uWhat < 2.5) gl_FragColor = texture(uWeather, vUv); else gl_FragColor = vec4(texture(uDepth, vUv).r, 0.0, 0.0, 1.0); }`,
        depthTest: false, depthWrite: false, toneMapped: false });
      probeRT = new THREE.WebGLRenderTarget(32, 32, { type: THREE.FloatType, format: THREE.RGBAFormat, minFilter: THREE.NearestFilter, magFilter: THREE.NearestFilter, depthBuffer: false });
    }
    const prev = renderer.getRenderTarget(), px = new Float32Array(4 * 32 * 32);
    for (const [k, w] of [['noise', 0], ['detail', 1], ['weather', 2], ['depth', 3]]) {
      probeMat.uniforms.uWhat.value = w; quad.material = probeMat;
      renderer.setRenderTarget(probeRT); renderer.render(fsScene, fsCam); renderer.readRenderTargetPixels(probeRT, 0, 0, 32, 32, px);
      const m = [0, 0, 0, 0]; for (let i = 0; i < 1024; i++) for (let c = 0; c < 4; c++) m[c] += px[i * 4 + c] / 1024;
      out[k] = m.map(v => +v.toFixed(4));
    }
    renderer.setRenderTarget(prev);
    return out;
  }
  const API = { S, init, update, draw, bakeStep, probe, get active() { return active(); }, get ready() { return ready; }, get layer() { return lay; }, get map() { return map; }, stats, get baked() { return bakeAt >= NB + 1; } };
  if (typeof window !== 'undefined') window.CLOUDS = API;
  return API;
})();
