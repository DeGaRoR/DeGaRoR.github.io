  ...
  function make(THREE, ctx) {
    ...
    function protosOf(c) {
      ...
      if (c.maps && c.maps.length) {                         // a flower: its pictures on cards
        ...
        // ONE MATERIAL PER MAP (G584): a rock's material is a plain copy made per part - the same values, the same
    ...
    function buildCell(cx, cz) {
      ...
      const cell = { n: 0, parts: new Map(), inst: [], by: {}, cx, cz };   // inst: [batch, id, ...] - the batched instances (G584)   // by: this cell's tally per species (STAT.by is the live sum)
      ...
      for (const it of items.values()) {
        ...
        // block; 0.5 m since G584 - the user: "shadows only for the kinds/sizes that show one (rocks > ~0.5 m)")
