...
function garageInit(api) {
  ...
  const SECTIONS = [
    ...
    { L: 'Engine, cowl & blades', led: ['engines'], items: [
      ...
      // bottom lines separately — that is what makes a flat-top Cub cowl and a
      // round-top Cessna with a chin scoop the same three sliders.
      { L: 'Cowl width',  p: 'cowl.halfW',         min: 0.05, max: 1.10, st: 0.01, u: ' m' },
      { L: 'Cowl top',    p: 'cowl.top',           min: 0.03, max: 1.10, st: 0.01, u: ' m' },
      { L: 'Cowl bottom', p: 'cowl.bot',           min: 0.03, max: 1.10, st: 0.01, u: ' m' },
      { t: 'sel', L: 'Intake', p: 'cowl.intake',
        o: () => Object.keys(GEN_INTAKES).map(k => [k, GEN_INTAKES[k].name]) },
      ...
      { t: 'sel', L: 'Blade material', p: 'prop.material',
        o: () => Object.keys(GEN_PROP_MATS).map(k =>
    ...
    { L: 'Wheels & suspension', led: ['gear'], items: [
      ...
      { L: 'Third tyre', p: 'gear.twR',           min: 0.05, max: 0.25, st: 0.01, u: ' m' },
      // NOT 'Camber': the wing section already has one, and two sliders with the
      // same name in one panel is a question the player should not have to ask
    ...
    { L: 'Glazing', led: [], items: [
      ...
      { t: 'sel', L: 'Wing bay', p: 'wings.0.centre',
        o: [['solid', 'Solid'], ['glass', 'Skylight'], ['open', 'Open (roof)']] },
      ...
      { L: 'Reach',           p: 'cabin.canopy.reach',  min: 0,    max: 1,    st: 0.02, u: '' },
    ...
    { L: 'Side windows', led: [], when: sp => get(sp, 'cabin.glazing') === 'windshield', items: [
    ...
    { L: 'Occupant', led: [], items: [
      ...
      { L: 'Torso recline',      p: 'cabin.pilot.lean',    min: -5,   max: 45,   st: 1, u: '°', sign: 1 },
      { L: 'Thigh below horiz',  p: 'cabin.pilot.thigh',   min: -20,  max: 40,   st: 1, u: '°', sign: 1 },
      { L: 'Shank below horiz',  p: 'cabin.pilot.shank',   min: 10,   max: 95,   st: 1, u: '°' },
      { L: 'Upper arm down',     p: 'cabin.pilot.armDown', min: -10,  max: 90,   st: 1, u: '°', sign: 1 },
      { L: 'Forearm down',       p: 'cabin.pilot.fore',    min: -45,  max: 60,   st: 1, u: '°', sign: 1 },
      { L: 'Head tilt',          p: 'cabin.pilot.head',    min: -25,  max: 25,   st: 1, u: '°', sign: 1 },
      { L: 'Arms in',            p: 'cabin.pilot.armIn',   min: -10,  max: 40,   st: 1, u: '°', sign: 1 },
      { L: 'Hip splay',          p: 'cabin.pilot.hipOut',  min: -5,   max: 35,   st: 1, u: '°', sign: 1 },
      { L: 'Knee splay',         p: 'cabin.pilot.kneeOut', min: -5,   max: 35,   st: 1, u: '°', sign: 1 },
      { L: 'Foot pitch',         p: 'cabin.pilot.ankle',   min: -25,  max: 40,   st: 1, u: '°', sign: 1 },
  ...
  // TOP-LEVEL GROUPS. Fifteen sections is more than a scroll can hold, so the
  // panel has two levels: a group per SUBSYSTEM, and inside it the sections
  // that were already there. The split is the aeroplane's, not the code's —
  // you set an airframe, then a powerplant, then a cabin — so a builder looking
  // for a knob knows which group it is in before they open anything.
  //
  // `led` stays on the SECTION, where the ledger keys already live: mass and
  // price belong next to the decision that incurs them, which is the whole
  // point of the badges. A group is a container and carries no ledger of its
  // own; its badge is the sum of its sections'.
  const GROUPS = [
    ...
    { L: 'Finish',   secs: ['Paint & finish'] },
  ];
  // Every section must land in exactly one group. A section named in no group
  // would silently vanish from the panel — its sliders still in the spec, just
  // unreachable — so the leftovers are collected rather than dropped.
  {
    const placed = new Set(GROUPS.flatMap(g => g.secs));
    const orphans = SECTIONS.filter(x => !placed.has(x.L)).map(x => x.L);
    if (orphans.length) GROUPS.push({ L: 'Other', secs: orphans });
  }
  const secByName = {};
  for (const x of SECTIONS) secByName[x.L] = x;
  ...
  // THE LOAD TEST, and the ORDER. You build, you load test, you fly — which is
  // the order a homebuilt actually goes in, and the reason ROLL OUT is not the
  // first button you can press. The bags go to +3.8 g limit and +5.7 g
  // ultimate; the wing bends on screen because the covering is bound to the
  // truss carrying the load. Rig: src/core/65_gen_loadtest.js, ticked by the
  // viewer here and by GATE LOAD headlessly — one computation, one verdict.
  const testBtn = document.createElement('button');
  testBtn.id = 'gTest'; testBtn.className = 'pri';
  testBtn.textContent = 'Load test the wing';
  const testOut = document.createElement('div');
  testOut.id = 'gTestOut'; testOut.className = 'gtest';
  testBtn.addEventListener('click', () => {
    const s = api.loadTestState && api.loadTestState();
    if (s && s.done) { api.endLoadTest(); syncRoll(); return; }   // back to the stand
    if (api.loadTest) api.loadTest();
    syncRoll();
    // poll while the bags go on, so the readout climbs with the load. Stops
    // itself the frame the rig finishes; no timer left running behind it.
    const poll = () => {
      const cur = api.loadTestState && api.loadTestState();
      if (!cur) return;
      syncRoll();
      if (!cur.done) requestAnimationFrame(poll);
    };
    requestAnimationFrame(poll);
  });
  rows.appendChild(testBtn);
  rows.appendChild(testOut);
  ...
  const syncRoll = () => {
    const inG = !api.inGarage || api.inGarage();
    const s = api.loadTestState ? api.loadTestState() : null;
    const testing = !!(s && !s.done);
    const tested = api.tested ? api.tested() : true;

    // While the rig is loaded the panel is a test readout, not a spec sheet.
    if (s) {
      const pct = v => v == null ? '—' : v.toFixed(2) + ' %';
      const cm = v => v == null ? '' : ' · ' + (v / 100 * s.semi * 100).toFixed(1) + ' cm';
      const y = s.worstPct == null ? '' :
        `<span class="${s.worstPct >= 100 ? 'bad' : 'good'}">` +
        `${s.worstPct.toFixed(0)}% of yield</span>`;
      testOut.innerHTML =
        `<b>${s.done ? (s.verdict || '') : '+' + s.n.toFixed(2) + ' g'}</b>` +
    ...
    testBtn.disabled = !inG || testing;
    testBtn.textContent = testing ? 'Loading the wing…'
                        : (s && s.done ? 'Back to the stand' : 'Load test the wing');

    // ROLL OUT waits for the test. Not a hard lock — it is your aeroplane and
    // you may fly an untested one — but the button says what you are doing.
    rollBtn.disabled = !inG || testing;
    rollBtn.textContent = !inG ? 'On the strip — flying'
                        : (tested ? 'Roll out to the strip'
                                  : 'Roll out untested');
    rollBtn.classList.toggle('warn', inG && !tested);
  };
  ...
  for (const G of GROUPS) {
    ...
    grpDet.className = 'ggrp'; grpDet.open = false;
    const grpSum = document.createElement('summary');
    const grpNm = document.createElement('span'); grpNm.textContent = G.L;
    const grpBd = document.createElement('i');        // the sum of its sections'
    grpSum.appendChild(grpNm); grpSum.appendChild(grpBd);
    grpDet.appendChild(grpSum);
    rows.appendChild(grpDet);
    badges['@' + G.L] = grpBd;

  for (const SEC of G.secs.map(nmm => secByName[nmm]).filter(Boolean)) {
    const det = document.createElement('details');
    // closed by default: a section header showing what that part weighs and
    // costs IS the most useful view of an aeroplane, and open-all runs several
    // screens deep. Open the one you are working on.
    ...
    for (const I of SEC.items) {
      ...
      if (I.t === 'chk') {
        // A real checkbox, not a two-option select. Five of these arrived with
        // the glazing, and a select reading "No / Yes" is a worse control than
        // the one the platform already has.
        const k = document.createElement('input');
        k.type = 'checkbox';
        k.checked = !!get(spec, I.p);
        k.addEventListener('change', () => { set(spec, I.p, k.checked); rebuild(); });
        row(det, I.L, k);
        els[I.p] = { k };
        continue;
      }
  ...
  function refresh() {
    ...
    if (built) {
      ...
      // Checkboxes re-read too, and they have to: clampSpec rewrites
      // `glazing: 'greenhouse'` as a bubble with `facet` set, so loading an old
      // spec turns a box on that the player never touched. A control that does
      // not show what was built is worse than no control.
      for (const I of SECTIONS.flatMap(x => x.items).filter(i => i.t === 'chk')) {
        const e = els[I.p]; if (e && e.k) e.k.checked = !!get(built, I.p);
      }
    }
    report();
    syncRoll();
  ...
  function report() {
    ...
    const L = s.ledger || {};
    const badge = (m, c) => (m >= 0.05 ? n(m, m < 10 ? 1 : 0) + ' kg' : '') +
                            (m >= 0.05 && c > 0 ? ' · ' : '') +
                            (c > 0 ? money(c) + ' cr' : '');
    for (const SEC of SECTIONS) {
      ...
      bd.textContent = badge(m, c);
      // A section whose controls do not apply to this build is HIDDEN rather
      // than left to be set with no effect. Side lights are the case: with a
      // full canopy the two openings meet, so they are a windscreen control.
      // (The build is still allowed to be wrong if the spec says so — this
      // hides the knob, it does not clamp the value.)
      const el = secEls[SEC.L];
      if (el && SEC.when) el.style.display = SEC.when(spec) ? '' : 'none';
    }
    // a group's badge is the sum of its sections': a container has no ledger
    for (const G of GROUPS) {
      const bd = badges['@' + G.L]; if (!bd) continue;
      let m = 0, c = 0;
      for (const nmm of G.secs) {
        const SEC = secByName[nmm]; if (!SEC) continue;
        for (const k of SEC.led) if (L[k]) { m += L[k].mass; c += L[k].cost; }
      }
      bd.textContent = badge(m, c);
    }
    ...
    read.innerHTML =
      ...
      // travel is the SPRING's own compression, not a node height — the leg is
      // raked into the firewall, so the axle moves along it, not under it.
      // And a FOLDED undercarriage reads healthier than a sound one on strain
      // (measured 1.4% folded against 4.8% working), so it gets its own line.
      cell('suspension travel', n(s.susTravel * 1000, 0) + ' mm',
           flag(!s.gearFolded)) +
      (s.gearFolded ? cell('GEAR FOLDED', n(s.susShift * 100, 0) + '% of its leg', 'bad') : '') +
      // The two legs AS BUILT. `main legs` is not always the slider: prop
      // clearance can demand a longer leg and win, so it says which rule bound
      // it — without that the slider looks broken through most of its range.
      cell('main legs', n(s.mainDrop, 2) + ' m',
           s.mainBoundBy === 'prop clearance' ? 'auto' : '') +
      cell('third leg', n(s.thirdLeg, 2) + ' m') +
      cell('camber', n(s.camber, 0) + '°') +
      cell('engine', n(s.hp, 0) + ' hp / ' + n(s.engineMass, 0) + ' kg') +
      // the prop's own numbers: what it sweeps, what it pulls standing still,
      // where its thrust runs out, and what it weighs at the front
      cell('prop', n(s.propD, 2) + ' m × ' + n(s.propBlades, 0) +
                   ' · ' + n(s.propMass, 1) + ' kg') +
      // the total the aeroplane pulls, not the per-disc figure — see propThrust
      cell('static thrust', n(s.propThrust, 0) + ' N · ' + n(s.propTW, 2) + ' T/W',
           flag(s.propTW > 0.28, s.propTW > 0.20)) +
      cell('thrust runs out', n(s.propV0 * 3.6, 0) + ' km/h',
           flag(s.propV0 > 1.25 * s.VCruise, s.propV0 > 1.08 * s.VCruise)) +
      // not a fault: a Cub's cylinders stick out on purpose. But it used to be
      // impossible to tell that from a cowl that had collapsed below its engine.
      cell('cowl', s.cowlEnclosed ? 'encloses engine'
                                  : 'engine out: ' + s.cowlOut.join('+')) +
