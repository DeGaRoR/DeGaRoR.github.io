...
//
// THE CRAFT AND THE FAR MAP: while its ground shadow lies inside the box
// (under 40 m AGL and a sun high enough not to throw it past the box's edge)
// the craft is on the near layer only - the far map's coarse copy of the same
// shadow would bloat the crisp edges (min is a union). Once the shadow leaves
// the box the craft joins layer 0 too, so its shadow on the ground far away
// exists, coarse, where coarse is all one can see. The main camera sees the
// craft through NEAR_LAYER, whatever layer 0 says. The field's near casters
// are tagged by render_world (every plain caster mesh; the instanced trees
// stay on the far map - a stand of 20 k-triangle trees is no 2048^2 pass).
// ============================================================
var SHADOW_NEAR = (function () {
  ...
  // follow(L, cg, sun, agl, snap): the box round the CG each frame, snapped to its own texels (render_world's snapToTexels);
  // the craft on layer 0 (the far map) only above aglFar
  ...
  function follow(L, cg, sun, agl, snap, camera) {
    ...
    if (!live) { if (craftGroup && craftFar !== true) { craftFar = true; craftGroup.traverse(m => m.layers.enable(0)); } return; }   // off: the craft back on the far map
    ...
    if (craftGroup && far !== craftFar) { craftFar = far; craftGroup.traverse(m => { if (far) m.layers.enable(0); else m.layers.disable(0); }); }
  }
  const API = { S, NEAR_LAYER, install, inject, make, tag, tagCraft, follow, get installed() { return installed; } };
