...
const GEN_RADIAL = 40;          // fuselage section resolution
const GEN_WHEEL_SEG = 24;       // wheel resolution, around
...
// SPINNER: a revolved ogive, apex -> base, in the [r/R, w/halfW] form
// genRevolveInto already takes. The cone-with-a-point it replaces was the worst
// single detail on the aeroplane — a black spike in front of the propeller.
const GEN_SPIN_SECT = [
  [0.00, -1.00], [0.20, -0.95], [0.38, -0.86], [0.55, -0.73],
  [0.70, -0.57], [0.83, -0.38], [0.92, -0.17], [0.97,  0.06],
  [1.00,  0.30], [1.00,  0.74], [0.94,  0.92], [0.55,  1.00], [0.00,  1.00],
];
...
function genSkin(def) {
  ...
  const skin = genMesh(), frame = genMesh(), strut = genMesh(), decal = genMesh(),
        ...
        // G5: glazing (interior / pane / frame), stacks, spinner, blade tips
        glass = genMesh(), gcabin = genMesh(), gframe = genMesh(),
        exhaust = genMesh(), spinner = genMesh(), proptip = genMesh(),
        // G6: the coaming and the instrument panel, off the windscreen fit line
        dash = genMesh(),
        // G7: the INSIDE of the covering — the same rows, stepped inward
        intr = genMesh(),
        liftstrut = genMesh(),
        // G6 GLAZING TRIALS: three independent routes, one spec option
        glazed = genMesh(), canopy = genMesh(), cframe = genMesh(), ctop = genMesh(),
        // G7: seats — cushion, piping, tube frame
  ...
  // Three treatments, because a beam is not one kind of thing. An EXTERNAL LIFT
  // STRUT gets GEN_STRUT_SECT with its chord fore-and-aft — that is what a lift
  // strut is, and the round bar it replaces is what made the bracing read as
  // scaffolding. Everything else is round, at 8 sides rather than 6: a hex bar
  // shows its flats wherever a highlight crosses it. Both ends are CAPPED —
  // computeVertexNormals has no face to average at an open rim, so every tube
  // end used to go dark.
  ...
  // sill depth as a fraction of the way from the top to the waterline, snapped
  // to a ring index — the snap is what makes the seam exact
  // SILL DEPTH is a ring INDEX again (user: the plane-based cut "makes it look
  // ugly"). A constant index is a constant angle round the section, which on a
  // straight-sided body IS a straight line and always lands exactly on ring
  // vertices — the snap is what makes the seam exact.
  const hsOf = () => hSill;    const hSill = Math.max(1, Math.min(GEN_RADIAL / 2 - 2,
  ...
  const cx1 = CN.x1 == null
    ? (S.cabin.glazing === 'windshield' ? ST[1].x
       : Math.min(fu.postX * 0.92, S.cab.noseGap + S.cab.len + 0.35))
    : CN.x1;
  ...
  const dsAt = k => Math.min(hsOf(bodyRows[Math.max(0, Math.min(bodyRows.length - 1, k))]),
                             hsOf(bodyRows[Math.max(0, Math.min(bodyRows.length - 1, k + 1))]));
  // ---- THE CORNER JOINT ------------------------------------------------
  ...
  if (CANOPY) {
    ...
    for (let k = kCut0; k <= kCut1; k++) {
      ...
      const dPrev = k > kCut0 ? dsAt(k - 1) : null;
      const dNext = k < kCut1 ? dsAt(k) : null;
  ...
  if (PN.on !== false && S.seating !== 'drone' && F.length > 2 && bodyRows.length) {
    ...
    // the opening's edge. It reads hsOf(), the very function the cut uses, so the
    ...
    const hsW = Math.max(1, Math.min(GEN_RADIAL / 2 - 1,
      Math.round(hsOf(rowW) * 2 * genClamp(PN.wrap == null ? 0.5 : PN.wrap, 0.15, 0.95))));
    const arc = [];
  ...
  // flex with whatever they are bolted to.
  {
    ...
    const pn = P.wf.L.F[Math.min(1, P.wf.L.F.length - 1)], pp = N[pn].p;
    const mast = [pp[0] + 0.12, pp[1] - 0.32, pp[2]];
    genTubeInto(strut, [pp[0] + 0.12, pp[1] - 0.10, pp[2]], mast, 0.010, 8, [[pn, 1]], B);
    genTubeInto(strut, mast, [mast[0] - 0.24, mast[1], mast[2]], 0.011, 8, [[pn, 1]], B);
  }
  ...
  wheel(P.TW, S.gear.twR);
  {
    ...
    // ---- THE PROPELLER ------------------------------------------------
    // Parametrised by the numbers that decide what a propeller DOES: diameter,
    // blade count, blade width and PITCH. Twist is not a styling curve — it is
    // the pitch, laid out by the propeller's own law
    //     beta(r) = atan(P / (2 pi r)),   P = pitch x D
    // so a coarse blade is coarse everywhere and the root/tip difference falls
    // out of the geometry instead of being drawn in. genPropPhys() reads the same
    // four numbers for the thrust model (60_gen_spec.js), so the propeller you
    // can see and the propeller the solver pulls on are one object.
    const PR = S.engines[0].prop || {};
    const R = S.propR;
    const nBl = Math.max(2, Math.min(5, PR.blades | 0 || 2));
    const pitch = genClamp(PR.pitch == null ? 0.55 : PR.pitch, 0.30, 1.05);
    const cFrac = genClamp(PR.chord == null ? 0.10 : PR.chord, 0.05, 0.20);
    const rootF = genClamp(PR.root == null ? 0.16 : PR.root, 0.08, 0.35);
  ...
  return {
    v: 5, generated: true,
    // DATA maps, not colour: the viewer must decode these LINEAR. See the
    // handoff note — one line in app.js's texture loop.
    linTex: ['bump', 'mr'],
    hub: B(hub),
    ...
    mats: {
      ...
      skin:      { tex: 'paint', nrm: 'bump', nrmScale: 0.9, mr: 'mr',
      ...
      ailR:      { tex: 'paint', nrm: 'bump', nrmScale: 0.9, mr: 'mr',
                   rough: 1 - S.paint.gloss },
      ailL:      { tex: 'paint', nrm: 'bump', nrmScale: 0.9, mr: 'mr',
                   rough: 1 - S.paint.gloss },
      flapR:     { tex: 'paint', nrm: 'bump', nrmScale: 0.9, mr: 'mr',
                   rough: 1 - S.paint.gloss },
      flapL:     { tex: 'paint', nrm: 'bump', nrmScale: 0.9, mr: 'mr',
                   rough: 1 - S.paint.gloss },
      elev:      { tex: 'paint', nrm: 'bump', nrmScale: 0.9, mr: 'mr',
                   rough: 1 - S.paint.gloss },
      rud:       { tex: 'paint', nrm: 'bump', nrmScale: 0.9, mr: 'mr',
                   rough: 1 - S.paint.gloss },
      vtR:       { tex: 'paint', nrm: 'bump', nrmScale: 0.9, mr: 'mr',
                   rough: 1 - S.paint.gloss },
      vtL:       { tex: 'paint', nrm: 'bump', nrmScale: 0.9, mr: 'mr',
                   rough: 1 - S.paint.gloss },
      // the cowl is a separate panel and reads as one: same paint, more gloss
      cowl:      { tex: 'paint', nrm: 'bump', nrmScale: 0.55, mr: 'mr',
      ...
      prop:      { color: 0x241f18, rough: 0.34, metal: 0.10 },
      proptip:   { color: 0xe8e1d1, rough: 0.30, metal: 0.05 },
      // A polished spinner needs something to reflect; at 0.92 metalness it went
      // black anywhere the sky was dim, so it is a bright semi-metal instead.
      ...
      glazed:     { tex: 'glaze', alphaTest: 0.45, rough: 0.07, metal: 0.0 },
      ...
      ctop:      { tex: 'paint', nrm: 'bump', mr: 'mr', nrmScale: 0.7,
                   rough: 1 - S.paint.gloss },
      ...
      intr:      { tex: 'cabin', rough: 0.86, metal: 0 },
