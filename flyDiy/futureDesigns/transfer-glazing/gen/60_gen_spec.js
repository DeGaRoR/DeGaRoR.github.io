...
const GEN_DEFAULT = {
  ...
  cabin: {
    ...
    // where the seats sit, as an OFFSET from what the cabin derives
    seatX: 0, seatY: 0.10, seatPitch: 0.86,
    // GLAZING (G6). `glazing` is the ROUTE the cabin transparency is built by —
    // texture projection, flat panels, or a canopy shell on top of the body —
    // and `glazeStyle` is what is drawn/shaped within the route. They are two
    // fields because the route is an engineering choice with different failure
    // modes (topology, sorting, distortion) and the style is a look.
    ...
    //   greenhouse  the shell faceted, with a bar down every facet edge
    glazing: 'bubble',
    ...
    // middle of the shell. x0/x1 are stations (m aft of the firewall) and null
    // derives them from the cabin, so the canopy follows the cabin sliders
    // unless it is pinned.
    ...
    // null means the windscreen run, i.e. the break line seen from the side.
    panel: { on: true, depth: 0.27, inset: 0.06, wrap: 0.50 },
    // THE OCCUPANT. A crash-test dummy on the first `pilots` seats. `stature`
    // is standing height in metres and everything else is a pose angle in
    // degrees; the rig is anthropometric, so the one length scales the figure.
    ...
    canopy: { x0: null, x1: null, height: 0, sill: 0.30, skew: 0.42,
              // `bubble` is the SECTION's fullness (1 = half-round blown canopy,
              // 0 = flat-sided with a crown). `lid` is a virtual clipping plane as
              // a fraction of the rise — 1 leaves the dome whole, lower slices the
              // top off flat. The wing lowers the plane further when it sits in the
              // way, which is the only thing a high wing now does differently.
              bubble: 0.70, lid: 1.0,
              // MAX WIDTH: how far the section swells sideways past the sill line.
              // 1 is flush with the body, 1.4 is a blown bubble standing proud of
              // it. Independent of height on purpose — the rise used to be capped
              // at the half-width, which silently tied the two knobs together.
              width: 1.0,
              // THE JOINT (0 = none). Chamfers the corner where the front bow
              // meets the sill rail, instead of letting it come to a downward
              // point. Frame detail only — the glass and its seam are untouched.
              ...
              facet: false,
              // side lights: a band of the same cut at the waistline, for a
              // WINDSCREEN build — see 63_gen_skin.js
              // SUNSCREEN: fraction of the canopy's arc that goes opaque, painted
              // like the fuselage. 0 = all glass.
              // SUNSCREEN: fraction of the arc that goes opaque, and where along the
              // window it starts — it must not reach the windscreen.
              sun: 0, sunStart: 0.38,
              // side lights: a band of the same cut at the waistline, for a
              // WINDSCREEN build. `sideTop` is where the band starts, both as
              // fractions of the half-section.
              sides: false, sideTop: 0.34, sideDepth: 0.5, sideReach: 1.0,
              // the door post: how far aft of the windscreen the side lights start
              sideGap: 0.10,
              // the windscreen's RAKE in degrees; null keeps the fuselage's own
              // windRun angle. The body's step and the canopy's front use it alike.
              wsAngle: null },
  },
  ...
  fuselage: {
          ...
          tailW: 0.10, tailBot: 0.20, tailTop: 0.38, tailY: 0,
  ...
  // THE PROPELLER is its own block, because its four numbers are the ones that
  // decide what it DOES, not just what it looks like. `D` null takes the
  // registry's diameter; everything else is a real design choice:
  //   blades  2..5
  //   pitch   P/D. 0.4 is a climb prop, 0.55 cruise, 0.9 a coarse cruise blade.
  //           It sets the twist law AND the speed at which thrust runs out.
  //   chord   blade width as a fraction of the RADIUS -> solidity -> static pull
  //   root    where the blade starts, as a fraction of the radius
  // The spinner is shape only, and cannot reach the cowl (see 63_gen_skin.js).
  engines: [{ type: 'a65_sensenich74', mount: 'nose', place: { dx: 0, dy: 0 },
              prop: { D: null, blades: 2, pitch: 0.55, chord: 0.10, root: 0.16,
  ...
  wings: [{ span: 10.0, chord: 1.60, taper: 1.0, dihedral: 3.0,
            ...
            // the carry-through over the cabin: covering, skylight, or only the
            // upper skin so the wing itself is the roof (a Cub's centre section)
            centre: 'solid',
  ...
  gear: { type: 'taildragger', suspension: 'bungee',
          ...
          place: { dx: 0, dtrack: 0 } },
  // `regX` is where the registration sits along the body, 0 forward of the wing
  // to 1 at the fin. Its SIZE is derived from the body, never stretched.
...
function clampSpec(spec) {
  ...
  cn.lid = genClamp(cn.lid == null ? 1 : cn.lid, 0.25, 1);
  cn.width = genClamp(cn.width == null ? 1 : cn.width, 0.85, 1.60);
  // THE CORNER JOINT. 'square' is the plain right angle where the opening ends;
  // 'chamfer' cuts it off on a straight diagonal, one ring index per slice, over
  // `jointRun` indices. See 63_gen_skin.js — the ramp has to be unit for the
  // diagonal to be exact.
  if (!['square', 'chamfer'].includes(cn.joint)) cn.joint = 'square';
  cn.jointRun = genClamp(cn.jointRun == null ? 3 : cn.jointRun, 1, 8);
  // SEATS. Pitch is a real distance between tandem seats (a cabin box shorter
  // than this simply has the seats overhang it, which is honest — it says the
  // cabin is too short). seatY is the squab above the cabin floor.
  ...
  pn.on = pn.on !== false;
  pn.depth = genClampN(pn.depth, 0.05, 1.20);
  pn.inset = genClamp(pn.inset == null ? 0.06 : pn.inset, 0, 0.25);
  pn.wrap = genClamp(pn.wrap == null ? 0.5 : pn.wrap, 0, 1);
  ...
  // the legs' OTHER axis: how far the knees splay from the centreline. thigh and
  // shank are flexion; these are abduction.
  pl.hipOut = genClamp(pl.hipOut == null ? 0 : pl.hipOut, -5, 35);
  pl.kneeOut = genClamp(pl.kneeOut == null ? 3 : pl.kneeOut, -15, 25);
  ...
  cn.sideTop = genClamp(cn.sideTop == null ? 0.17 : cn.sideTop, 0.05, 0.75);
  cn.sideDepth = genClamp(cn.sideDepth == null ? 0.64 : cn.sideDepth, 0, 1);
  cn.sideReach = genClamp(cn.sideReach == null ? 0.24 : cn.sideReach, 0.2, 1);
  // the windscreen's middle line: -1 concave, 0 the plain smoothstep, +1 convex
  cn.wsCurve = genClamp(cn.wsCurve == null ? 1 : cn.wsCurve, -1, 1);
  cb.seatX = genClamp(cb.seatX == null ? 0.41 : cb.seatX, -0.45, 0.45);
  cn.x0 = genClampN(cn.x0, 0, 8); cn.x1 = genClampN(cn.x1, 0, 9);
  // a windscreen-only cut is covered flush: zero stand-off is the definition
  ...
  for (const e of S.engines) {
    ...
    const pr = e.prop || (e.prop = {});
    pr.blades = genClamp(pr.blades == null ? 2 : pr.blades | 0, 2, 5);
    pr.pitch = genClamp(pr.pitch == null ? 0.55 : pr.pitch, 0.30, 1.05);
    pr.chord = genClamp(pr.chord == null ? 0.10 : pr.chord, 0.05, 0.20);
    pr.root = genClamp(pr.root == null ? 0.16 : pr.root, 0.08, 0.35);
    pr.D = genClampN(pr.D, 0.8, 4.2);
    const sn = pr.spinner || (pr.spinner = {});
  ...
  // TAIL HEIGHT. Not a new dimension: the tail-end section is already fully
  // described by its floor and its deck, so this slides BOTH together and keeps
  // the depth it has. That is the one control the shape was missing — run the
  // tail up and the top line is flat from cabin to fin, run it down and the
  // BOTTOM line is; anywhere between is a tapered boom. Applied here rather than
  // in the frame so nothing downstream learns a new field: 61_gen_frame.js still
  // reads only tailBot/tailTop, and clampSpec works on a CLONE, so the offset
  // cannot accumulate the way gear.track once did.
...
// ---------------------------------------------------------------------------
// genPropPhys(S) — the propeller's THRUST NUMBERS from its geometry, so the
// blade you can see and the blade the solver pulls on are one object. Scaled off
// the registry entry rather than computed from scratch: the registry value is a
// measurement of a real propeller on a real engine, and this only says how far
// THIS blade is from it.
//
//   Tstatic  disc area x solidity. Static thrust goes as D^2 and, more weakly,
//            as the blade area in it — sigma^0.7 is the usual exponent. A coarse
//            blade stalls at zero airspeed, hence the mild pitch penalty.
//   kV2      not free: thrust runs out at the speed the propeller screws itself
//            forward at, V0 = n P. The engine sets n, so V0 goes as the PITCH
//            IN METRES — pitch x D — and kV2 = Tstatic / V0^2 closes it.
//
// Reference blade: 2 blades, chord 0.10 R, pitch 0.55, i.e. the registry's own.
// At those numbers this returns the registry values exactly.
function genPropPhys(S) {
  const PP = (typeof POWERPLANTS !== 'undefined' && POWERPLANTS[S.engines[0].type]) || null;
  if (!PP) return null;
  const p = S.engines[0].prop || {};
  const D0 = PP.prop.D, D = p.D || D0;
  const nB = p.blades == null ? 2 : p.blades, cF = p.chord == null ? 0.10 : p.chord;
  const pit = p.pitch == null ? 0.55 : p.pitch;
  const sig = nB * cF, sig0 = 2 * 0.10;
  const Tstatic = PP.prop.Tstatic * Math.pow(D / D0, 2)
                * Math.pow(sig / sig0, 0.70) * Math.pow(0.55 / pit, 0.25);
  const V0 = Math.sqrt(PP.prop.Tstatic / PP.prop.kV2) * (pit * D) / (0.55 * D0);
  return { D, Tstatic, kV2: Tstatic / Math.max(1e-6, V0 * V0), V0, solidity: sig };
}
...
function resolveSpec(spec) {
  ...
  const propR = 0.5 * ((S.engines[0].prop && S.engines[0].prop.D) || (PP ? PP.prop.D : 1.8));
  S.propR = propR;
