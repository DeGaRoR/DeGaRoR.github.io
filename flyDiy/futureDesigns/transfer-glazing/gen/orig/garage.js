...
function genPaintDataURI(spec) {
  ...
  // The REGISTRATION is no longer painted here. Body `u` is normalised angle
  // and the circumference shrinks aft, so a glyph of fixed u-width narrowed
  // toward the tail — "the writing is all squeezed at the back". It is a decal
  // now (genRegDataURI, below, on the `decal` group from 63_gen_skin.js), which
  // carries its own undistorted 0..1 grid.
  return c.toDataURL('image/png');
}

// The registration sheet: transparent, one line of text, read left to right.
// The decal's own UV is a plain grid, so what is drawn here is what appears on
// the aeroplane — no parameterisation in the way.
function genRegDataURI(spec) {
  ...
  // the decal grid runs nose-to-tail in v, so the text is laid along v and the
  // patch is wider than tall — rotate it into the sheet
  g.save();
  g.translate(W / 2, H / 2);
  g.fillStyle = hex(spec.paint ? spec.paint.trim : 0x1b3a5c);
  g.font = '700 74px "IBM Plex Mono", monospace';
