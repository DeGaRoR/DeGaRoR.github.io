...
console.log('7b. several decks + the look: the march (static)');
{
  ...
  yes(/PA\[o \+ 2\] = T\.erode \* S\.erodeK/.test(cj) && /LA\[o \+ 3\] = per\(S\.period > 0 \? S\.period : T\.period\)/.test(cj), 'erode and period are the type\'s, scaled / overridden by the dials (G460.4: rounded to divide the span, so the shadow tile wraps without a seam)');
  yes(/const per = q => map\.span \/ Math\.max\(1, Math\.round\(map\.span \/ q\)\)/.test(cj) && /map\.span \/ Math\.max\(1, Math\.round\(map\.span \/ S\.detailPeriod\)\)/.test(cj), 'the deck noise and the detail noise tile over the span (no seam at the shadow tile\'s wrap)');
  ...
  { const RP = src('viewer/render_premises.js'), RW = src('viewer/render_world.js'), PV = src('viewer/pattern_vis.js'), HGs = fs.readFileSync(path.join(__dirname, '_house_gen.js'), 'utf8');
    yes(/const LAMPS = \{ pool: \[\], pub: \[\], glass: new Map\(\), smoke: new Set\(\), glowKeys: new Set\(\), gain: 2/.test(RP) && /for \(let i = 0; i < LAMPS\.N; i\+\+\) \{ const l = new THREE\.PointLight/.test(RP) && /N: 8, reach: 500/.test(RP), 'a CONSTANT pool of eight point lights on the premises root, assigned within 500 m');
  ...
  // G456: the fixtures' own glow - the lit props' emissive by the day, every placement of a key together
  { const PJ = src('viewer/props.js'), RP = src('viewer/render_premises.js');
    yes(/function propSetGlowOf\(key, k\)/.test(PJ) && /if \(m\.userData\.emis0 == null\) m\.userData\.emis0 = m\.emissiveIntensity;/.test(PJ) && /for \(const l of propLevels\(key\)\) \{ PROP_GLOW\.set\(l\.key, k\);/.test(PJ) && /const glow = PROP_GLOW\.get\(key\);\r?\n  if \(glow != null\) glowMaterials\(mats, glow\);/.test(PJ) && /propLevels, propSetGlowOf \};/.test(PJ), 'props.js publishes propSetGlowOf: the author\'s intensity kept, the levels of detail with it, a prop built later takes the last k');
    yes(/if \(L\.prop\) LAMPS\.glowKeys\.add\(L\.prop\);/.test(RP) && /for \(const key of LAMPS\.glowKeys\) propSetGlowOf\(key, kFix\);/.test(RP) && /on \* Math\.pow\(0\.92 \/ Math\.max\(0\.92, ex\), 0\.9\)/.test(RP) && /for \(const key of LAMPS\.glowKeys\) propSetGlowOf\(key, 0\);/.test(RP), 'the pool drives the published fixtures\' glow by the day (the lenses\' law) and the mute takes it to nothing'); }
