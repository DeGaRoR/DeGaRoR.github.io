...
// capsize honestly).
//
// Node: require('./_hydro_gen.js'); browser: window.HYDRO_GEN.
'use strict';
(function () {
...
function hydroForces(F, S, water, out, opt = {}) {
  const P = F.P, rho = P.rho, V = F.V, t = S.t;
  const W = out.W, D = out.d;
  const R = S.R, p = S.p;
  const xhat = [R[0], R[3], R[6]];               // the float's aft axis, in world
  // 1. world vertices; float-frame x (aft of the step) is the local x itself
  for (let i = 0; i < V.length; i++) { const q = sub(V[i], F.cg); matVec(R, q, W[i]); add(W[i], p, W[i]); }
  // 2. the step edge: its world height, its flow speed, the air that reaches it
  const eK = W[F.edge.K];
  const vE = add(S.v, cross(S.w, sub(eK, p)));
  ...
  for (let k = 0; k < F.panels.length; k++) {
    ...
    if (pn.kind === 'bottomF' || pn.kind === 'bottomA') {
      ...
      for (let j = 0; j < o.poly.length; j++) {
        ...
        const ql = matTVec(R, sub(q, p)); add(ql, F.cg, ql);
  ...
  for (let k = 0; k < F.panels.length; k++) {
    ...
    sub(o.c, p, r); cross(S.w, r, vc); add(vc, S.v, vc);
    ...
    const cl = matTVec(R, r); add(cl, F.cg, cl);
