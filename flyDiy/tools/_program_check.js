#!/usr/bin/env node
// _program_check.js — GATE PROGRAMS (G580): the programs a boot links are the same every boot, and the
// roll-out's compile step warms the ones the first frames draw with.
//
// The warm roll-out spent 16-26 s in its compile step (futureDesigns/PERF-2026-09-23.md G580). A census of
...
function warmThenFrame(useOld) {
  ...
  R.compile(sc, cam);
  if (useOld) {   // THE CONTROL: app.js's warm-up before G580, verbatim in effect
