...
PAGE.post = ctx => {
  ...
  // THE RIB CAPS ARE THEIR OWN CLASSES (A7, 2026-09-21, the user: "the flat
  // face at the end of the wing with the cut parameter, and the central
  // portion with the cut central section ... strange mapping or no mapping"
  // - "a full longeron or at least a bolted metal plate" at the root, "what
  // is proper on the wing far end"). genCapLoft closes the loft's first and
  // last rows with a fan in the SKIN mesh: the face's vertices are the
  // row's, so its field has one span coordinate across the whole face and
  // the grammar smears; and welded to the skin, computeVertexNormals rolled
  // the edge - the face read as a soft bevel. A cap is a primitive whose
  // vertices all sit at one span station: root (the cut centre's face) or
  // tip (a square or clipped tip's end rib). Each takes its own bucket -
  // its own vertices, a hard edge - and its own dressing below.
  const zTipEnd = (() => { let m = 0; for (let i = 0; i < gs.skin.nv; i++) {
    const az = Math.abs(toBody([gs.skin.pos[i*3], gs.skin.pos[i*3+1], gs.skin.pos[i*3+2]])[2]); if (az > m) m = az; } return m; })();
  // ...and only where the face SHOWS: a cut-away centre's root faces, a
  // square or clipped tip's end rib. Every other cap stays in its skin
  // class (GATE CLIP measures fittings against closed skins; a lone fan
  // inside a solid centre section is not a surface anything sits on).
  const capsRoot = (['solid', 'glass', 'open', 'cutout', 'foreCut', 'topGlass', 'topFuselage', 'removed'][Math.round(P.wgCentre) || 0]) === 'removed';
  const capsTip = ['square', 'clipped'].includes(TIP_KEYS[Math.round(P.wgTip) || 0]);
  const capOf = vs => {
    if (!vs || vs.length < 3 || (!capsRoot && !capsTip)) return null;
    let lo = 1e9, hi = -1e9;
    for (const i of vs) {
      const az = Math.abs(toBody([gs.skin.pos[i*3], gs.skin.pos[i*3+1], gs.skin.pos[i*3+2]])[2]);
      if (az < lo) lo = az; if (az > hi) hi = az;
    }
    if (hi - lo > 2e-4) return null;              // a skin face spans stations
    if (capsRoot && Math.abs(lo - zRoot) < 2e-3) return 'capRoot';
    if (capsTip && Math.abs(hi - zTipEnd) < 2e-3) return 'capTip';
    return null;
  };
  ...
  function lensStrip(geo, P, FS) {
    ...
    const adj = new Map();
    for (const e of seen.values()) if (e.n === 1) {
  ...
  if (gs.skin) {
    ...
    // THE CAPS (A7): the root face of a cut centre is a BOLTED PLATE - the
    // wing skin's own finish on a triplanar material (no field to smear), a
    // ring of bolt heads 22 mm inside its outline at ~55 mm - and a square
    // or clipped tip's end rib takes the same plate with the skin's rivets
    // at ~40 mm, the tip rib's own flange. A rounded tip's last row is a
    // point and gets no fan worth dressing. Only a cut centre shows its
    // root face; every other centre kind covers it.
    {
      const CTRK = ['solid', 'glass', 'open', 'cutout', 'foreCut', 'topGlass', 'topFuselage', 'removed'][Math.round(P.wgCentre) || 0];
      const K = window.GEAR_KIT;
      const plateMat = (window.CAGE_SECMAT && window.CAGE_SECMAT('wingSkin', { surf: 0, fieldM: 1, struct: 0, side: THREE.DoubleSide })) || wingMat('main');
      for (const [cl, pitch, rB, lB] of [['capRoot', 0.055, 0.0045, 0.006], ['capTip', 0.040, 0.0022, 0.003]]) {
        const pr = parts[cl];
        if (!pr) continue;
        if (cl === 'capRoot' && CTRK !== 'removed') { add(parts, cl, wingMat('main')); continue; }
        // the root's plate is BARE ALLOY (the fitting the wing bolts to, not
        // paintwork); the tip's end rib is painted with the wing
        const o = add(parts, cl, cl === 'capRoot' && GG && GG.gearMat ? GG.gearMat('alloy') : plateMat);
        if (!o || !K) continue;
        // the rim: the face's vertices less its fan centre, ordered round the
        // centroid in the face's own plane; one cap per side
        const P3 = pr.geo.attributes.position, nv = P3.count;
        // (the parts are in CAGE space: x is the span, port positive)
        const byZ = new Map();
        for (let i = 0; i < nv; i++) {
          const p = [P3.getX(i), P3.getY(i), P3.getZ(i)];
          const k = Math.round(p[0] * 1e3);
          if (!byZ.has(k)) byZ.set(k, []);
          byZ.get(k).push(p);
        }
        const bag = K.Bag();
        for (const pts of byZ.values()) {
          if (pts.length < 6) continue;
          const c = [0, 0, 0];
          for (const p of pts) { c[0] += p[0] / pts.length; c[1] += p[1] / pts.length; c[2] += p[2] / pts.length; }
          // the face normal: along the span - outward at a tip, toward the
          // centreline at a cut root (the face looks into the cut)
          const nrm0 = [cl === 'capTip' ? Math.sign(c[0] || 1) : -Math.sign(c[0] || 1), 0, 0];
          // the rim polygon by angle in the plane; drop the centre (nearest the centroid)
          const u = K.nrm(K.sub(pts[0], c)), w = K.nrm(K.crs(nrm0, u));
          const rim = pts.map(p => { const d = K.sub(p, c); return { p, r: Math.hypot(K.dot(d, u), K.dot(d, w)), a: Math.atan2(K.dot(d, w), K.dot(d, u)) }; })
            .filter(q => q.r > 0.01).sort((p, q) => p.a - q.a);
          if (rim.length < 6) continue;
          // walk the rim at the pitch, each fastener 22 mm inside
          let acc = 0, last = rim[rim.length - 1].p;
          for (const q of rim) {
            acc += K.len(K.sub(q.p, last)); last = q.p;
            if (acc < pitch) continue;
            acc = 0;
            const inward = K.nrm(K.sub(c, q.p));
            const at = K.add(q.p, K.mul(inward, cl === 'capRoot' ? 0.022 : 0.012));
            K.bolt(bag, K.add(at, K.mul(nrm0, 0.0005)), nrm0, rB, lB);
          }
        }
        const hm = (GG && GG.gearMat) ? GG.gearMat('steel') : plateMat;
        const bm = bag.mesh(group, hm);
        if (bm) bm.name = 'edCap_' + cl;
      }
    }
    ...
    for (const sd of ['R', 'L']) {
      ...
      o.renderOrder = 1003;
      // G316 (the user: "the wing light ... their transparent part is the
      // one getting exploded, and this one should get the same joint
      // treatment as the windows to highlight its contour. All the same
      // settings"): the lens unbolts FORWARD off the D-nose by the explode
      // distance (the bay and its lamp stay in the wing), and the hole it
      // came out of wears the windows' joint bead — the cage's own `rimW`
      // gauge, the `rimWin` switch, the `joint` section's material — laid
      // along the cut's own boundary loop, on the skin, where it stays.
