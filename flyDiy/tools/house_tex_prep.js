...
//
// kind: 'plank' | 'veneer' | 'roof' | 'log' | 'plain' | 'stone' | 'end' | 'steel' | 'render'
//
...
const SETS = [
  ...
  // G392: PAINTED RENDER - its own kind, offered to the wall role only (an
  // institution's stucco); the paint map is what the tint lands on
  ['render', 'render', 'painted render', 2.0, 512, 0.00, true, false],
  // THE END OF A LOG (G273): generated by tools/house_endgrain_gen.py, nine
  // log ends per metre tile, mapped pith-centred by the generator. Its own
  // kind, `end`, so no role is ever offered it: it is not a covering.
  ['endgrain', 'end', 'log ends', 1.25, 512, 0.00, false, false],
];
