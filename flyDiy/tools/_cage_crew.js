...
function buildThrottleDash(g0, A, P, sx) {
  ...
  const LT = P.thrLen != null ? P.thrLen : 0.16;
  // G446 (A3): A TWIN GETS TWO RODS, 56 mm apart inboard (two 52 mm knobs
  // side by side), each on its engine's drive - the wall lever's rule
  // (twinEngines / thr0, thr1); a single engine keeps its one rod on 'thr'
  const inb = sx >= 0 ? -1 : 1;
  const rods = twinEngines(P) ? [['edCtl_throttle', 'thr0', 0], ['edCtl_throttle2', 'thr1', inb * 0.056]] : [['edCtl_throttle', 'thr', 0]];
  let first = null;
  for (const [nm, drv, dx] of rods) {
    const xR = xT + dx;
    const z0 = F && F.depthAt ? F.depthAt(xR, yT) - 0.001 : A.zDash + 0.005;
    if (KD) {
      // a plated collar with a hex lock nut on the panel's face (session 4e)
      const col = KD.Bag();
      KD.revolve(col, [xR, yT, z0], [0, 0, -1], [[0.020, 0], [0.020, 0.003], [0.011, 0.005], [0.011, 0.016], [0.0085, 0.017], [0, 0.017]], 24, false);
      KD.sweep(col, [[xR, yT, z0 - 0.005], [xR, yT, z0 - 0.011]], () => { const o = []; for (let i = 0; i < 6; i++) { const a = i * Math.PI / 3; o.push([0.0135 * Math.cos(a), 0.0135 * Math.sin(a)]); } return o; }, true, [0, 1, 0]);
      col.mesh(g, M.plated);
      ...
      if (window.CAGE_PANEL && window.CAGE_PANEL.material) {
        ...
        ao.position.set(xR, yT, z0 - 0.0003); ao.rotation.y = Math.PI; ao.renderOrder = 2;
        g.add(ao);
      }
    } else {
      const m = new THREE.Mesh(
        new THREE.CylinderGeometry(0.020, 0.020, 0.012, 12), M.metal);
      m.position.set(xR, yT, z0 - 0.006); m.rotation.x = Math.PI / 2;
      g.add(m);
    }
    ...
    const kp = [xR, yT, z0 - LT * 0.72];
    const gT = movingAt(g, nm, kp, [0, 0, 1], drv, 0, 0,
                        { slide: [0, 0, LT * THR_PUSH], slideDrive: drv,
                          slideSgn: 1 });
    tube(gT, KD ? M.plated : M.metal, inG(gT, [xR, yT, z0]), inG(gT, [xR, yT, z0 - LT * 0.60]),
         0.006);
    if (KD) {
  ...
  return { obj: gripAt(first, [0, 0.004, -0.008], [1, 0, 0]),
           label: 'throttle (push-pull)', twinDx: rods.length > 1 ? rods[1][2] : 0 };
}
...
function buildConsole(g0, A, P, cx, withQuadrant, box) {
  ...
  // G446 (A3): a twin gets two levers between the cheeks, 24 mm apart, each on its engine's
  // drive (the wall lever's rule); the boss is one across the box either way
  const twin = twinEngines(P);
  const levers = twin ? [['edCtl_throttle', 'thr0', -0.012], ['edCtl_throttle2', 'thr1', 0.012]] : [['edCtl_throttle', 'thr', 0]];
  const hw = twin ? 0.009 : 0.012;
  let gT = null, K = null;
  for (const [nm, drv, dx] of levers) {
    const pv = [piv[0] + dx, piv[1], piv[2]];
    const gL = movingAt(g, nm, pv, [1, 0, 0], drv, 1, THR_ARC);
    ...
    if (KQ) {
      ...
      KQ.sweep(lev, [[0, 0, 0], KL], () => [[-hw, -0.004], [hw, -0.004], [hw, 0.004], [-hw, 0.004]], true, [1, 0, 0]);
      if (!gT) KQ.revolve(boss, [-dx, 0, 0], [1, 0, 0], [[0.011, -0.038], [0.011, 0.038], [0, 0.038]], 16, true);
      KQ.revolve(kb, KL, _nrm3([kn[0] - piv[0], kn[1] - piv[1], kn[2] - piv[2]]), ballProf(twin ? 0.020 : 0.024, 0.011), 36, true);
      lev.mesh(gL, M.frame); boss.mesh(gL, M.plated); kb.mesh(gL, M.ball);
    } else { tube(gL, M.metal, [0, 0, 0], KL, 0.008); ballAt(gL, M.knob, KL, 0.024); }
    if (!gT) { gT = gL; K = KL; }
  }
...
PAGE.post = ({ scene, spec, mesh, P, stat }) => {
  ...
  if (thrMode === 1 && thr && thr.obj && thr.obj.parent && thr.obj.parent.position) {
    ...
    const dx2 = thr.twinDx || 0;                         // G446: the twin's second rod, inboard
    A.keepOut.push({ x0: Math.min(p.x, p.x + dx2) - m, x1: Math.max(p.x, p.x + dx2) + m, y0: p.y - m, y1: p.y + m + out * 0.5, what: 'throttle' });
  }
