const { buildDC3, makeSim, makeAutopilot, makeWorld } = require('./flight_core.js');
const world = makeWorld();
const def = buildDC3();
const sim = makeSim(def, world);
sim.reset(0);
for (let i = 0; i < sim.n; i++) { sim.p[i*3+2] += 1.5; sim.v[i*3+2] += 0.002*Math.sin(i*2.7); }
for (let s0 = 0; s0 < 10*60; s0++) sim.step(1/60);
const ap = makeAutopilot(sim, def);
const iW0 = def.nodes.findIndex(n => n.tag === 'WF' && Math.abs(n.p[2] - 7.6) < 0.2 && n.p[2] > 0);
const iW1 = def.nodes.findIndex(n => n.tag === 'WF' && Math.abs(n.p[2] - 14.3) < 0.2 && n.p[2] > 0);
let smaxCh = 0, smaxGr = 0, lastPhase = '', log = [], rollPitchMin = 99;
let flapMin = 99, flapMax = -99, deP = 0, chat = 0, chatN = 0;
for (let s0 = 0; s0 < 460*60; s0++) {
  const t = s0/60;
  ...
  if (sim.stats().bad) { console.log(`NaN t=${t.toFixed(1)} ${ap.phase}`); console.log('GATE DC3: FAIL (NaN)'); process.exit(1); }
...
console.log(`STOP: x=${cg[0].toFixed(0)} z=${cg[2].toFixed(1)} rolloutPitchMin=${rollPitchMin.toFixed(1)} strain ch=${(smaxCh*100).toFixed(1)}% gr=${(smaxGr*100).toFixed(0)}%`);
console.log(`WING: flap ${flapMin.toFixed(1)}..${flapMax.toFixed(1)} deg (dihedral 5 outboard) | chatter ${(chat/Math.max(1,chatN)*57.3*60).toFixed(1)} deg/s`);
const pass = td && td.sink < 1.8 && Math.abs(td.z) < 5 && td.x > -1060 && td.x < -700
  && cg[0] < 20 && Math.abs(cg[2]) < 9 && smaxCh < 0.06 && smaxGr < 0.40
  && rollPitchMin > 0 && flapMin > 0 && flapMax < 12 && chat/Math.max(1,chatN)*57.3*60 < 6;
console.log(pass ? 'GATE DC3: PASS' : 'GATE DC3: FAIL');
