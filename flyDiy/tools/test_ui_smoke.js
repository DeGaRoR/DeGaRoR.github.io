...
try {
  ...
  // in order), nothing was left pending, three quiet frames tore it down
  {
    ...
    const want = ['treeBins', 'aircraft', 'garage', 'editor', 'sync', 'parked', 'restore', 'compile', 'firstFrame'];   // compile: LOADING S2 (sync here: the stub renderer has no compileAsync); parked: G411 (a no-op without the world pack); treeBins: S3 (the world scene builds under the roll-out screen)
