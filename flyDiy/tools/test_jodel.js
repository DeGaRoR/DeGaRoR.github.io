const { buildJodel, makeSim, makeAutopilot, makeWorld } = require('./flight_core.js');
const world = makeWorld();
const def = buildJodel();
...
const iW0 = def.nodes.findIndex(n => n.tag === 'WF' && Math.abs(n.p[2] - 2.10) < 0.2 && n.p[2] > 0);
const iW1 = def.nodes.findIndex(n => n.tag === 'WF' && Math.abs(n.p[2] - 4.36) < 0.2 && n.p[2] > 0);
...
for (let s0 = 0; s0 < 300*60; s0++) {
  ...
  if (sim.stats().bad) { console.log(`NaN t=${t.toFixed(1)} ${ap.phase}`); console.log('GATE JODEL: FAIL (NaN)'); process.exit(1); }
...
console.log(`AILE (diedre 14): battement ${flapMin.toFixed(1)}..${flapMax.toFixed(1)} deg | gouverne ${(chat/Math.max(1,chatN)*57.3*60).toFixed(1)} deg/s`);
const pass = td && td.sink < 1.2 && Math.abs(td.z) < 5.5 && td.x > -720 && td.x < -420
  && cg[0] < 20 && Math.abs(cg[2]) < 7 && smaxCh < 0.06 && smaxGr < 0.40
  && rollPitchMin > -2 && flapMin > 8 && flapMax < 31 && chat/Math.max(1,chatN)*57.3*60 < 6;
console.log(pass ? 'GATE JODEL: PASS' : 'GATE JODEL: FAIL');
