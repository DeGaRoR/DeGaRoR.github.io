...
## G34 — WING STRUCTURE, STRUTS ON THE CAGE, MASS/CG (2026-08-27)

The G30 recorded list's technical three, done together; the fourth —
the cage-style outline/creases rework — is the user's, and is what
remains of P2.

**STRUCTURE BY THE SAME FILTER, >=1-VERTEX.** Spars, ribs, carry-through
and the strut fan are the `frame` group's wing-bound faces — every
internal beam is a tube whose vertices bind exclusively to their end
node, so the skin's binding filter works there too, with the rule
inverted from ALL to ANY: a carry-through or fan tube binds one end to a
fuselage node, and demanding every vertex would drop the whole member.
(Its far cap drops instead — an open tube end inside the body.) The node
set gains the cantilever box's lower caps (parts.wf FB/RB), which the
skin filter never needs — the covering binds to the upper spars alone.
Measured on the default: 1824 structure faces under the strut fan, 4832
under the cantilever box.

**THE WING JOINS THE THREE-FAMILY TRANSPARENCY RULING** (the view
panel's, 2026-08-19): the covering — main skin, centre, tips, ailerons,
flaps, the glass centre — is exterior skin and fades with `fuselage α`;
the structure fades with `structure α`, wearing the game's own frame
paint (0x5a6470), wire mode included. Struts and the pitot are external
hardware and stay solid (the rod rule: fading the skin must leave the
visible structure standing). Same transparent/opacity/depthWrite
treatment as the cage's matOf, so the two skins fade as one surface —
and ZERO new UI: the dial that already reveals the fuselage truss now
reveals the wing's.

**STRUTS ROOT ON THE LIVE CAGE** (G30's recorded honest divergence,
closed). The game's strut root is a frame corner of the fuselage it
generated and discarded; the fitting belongs on the live skin. The root
node projects to the nearest cage-surface point at its own station
(angle guess from the section's normalised coords, then a shrinking
bracket — the section is not polar-uniform), and the delta rides
per-vertex ON THE BINDING: a tube vertex binds exclusively to its end
node, so the root ring, its cap, AND the internal fan members that
converge on the same node all move together, and the wing end never
moves. Measured on the jodel: the root lands 0.03 mm off the skin, both
sides; a low wing roots UP (+0.53 m above the keel-band wing) — the
strut direction inverts with position exactly as the game intends.

**MASS & CG, THE GENERATOR'S OWN NUMBERS.** Mass is the ledger's
`wings` + `bracing` sections (bracing is wing-only in 61_gen_frame.js),
so spars, box or fan, ribs, covering and struts arrive with no second
bookkeeping to drift. The CG sums the wing's own nodes — clean of
payload because the default spec keeps fuel in the NOSE tank; a
wing/panel tank would land on the spar roots, declared in the layer so
a future tank dropdown knows to revisit. Reported as %MAC off the
spec's own cBar/xAC: the numbers P3's declared join will read. Measured:
85 kg at span 10, 71.7 at 8, 99.3 at 12 — the slider moves the number
(ruling 3) — cantilever box 131 kg (the box costs mass, honestly),
CG 40% MAC across all of them (structure-dominated, spars at 22/65%).
The stat reports the BUILT bracing (`def.parts.bracing`), not the
requested one — the crank/short-offset substitution stays non-silent
(verified: crankAt 0.45 with struts requested prints `cantilever box`).
FOUND IN PASSING, fixed: the stat read `geom.S` but the field is
`geom.Sw` — the area had never printed since G30 (15.5 m² now shows on
the default).

VERIFIED: FIT / FIN / COWL CHECK / ENG CHECK / ENGMESH all OK; zero
console errors through the whole rebuild matrix (high/low, strut/
cantilever/crank, spans 8-12, fades, wire); skin extraction untouched —
1840 faces on the default, G30's own number. The Browser pane would not
composite, so the render proof is SAMPLED, not squinted (the standing
cure): differential pixel counts with the part hidden vs shown — struts
1006 px, structure 5959 px on the full-frame view. NOT DONE (recorded):
the strut root lands on the skin but carries no fitting hardware yet
(pad/lug/bolts — the gear layer's fitPad is the pattern when wanted);
the wing wireframe is still wing-layer-only (the G32 note stands); the
outline/creases rework is the user's, and closes P2.
